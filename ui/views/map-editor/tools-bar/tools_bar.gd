...
func _on_building_tool_button_pressed() -> void:
	if _is_active_tool(MapTools.MapEditorTool.PLACE_ROADSIDE):
		if _ui_manager.is_ui_view_visible(RoadSideObjectPanel.VIEW_NAME):
			_map_interactions_manager.set_active_tool(MapTools.MapEditorTool.NONE)
			return
